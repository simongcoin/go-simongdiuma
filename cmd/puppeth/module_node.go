...
{{end}}
RUN \
  echo 'gsmc --cache 512 init /genesis.json' > gsmc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.simongdium/keystore/ && cp /signer.json /root/.simongdium/keystore/' >> gsmc.sh && \{{end}}
	echo $'exec gsmc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gsmc.sh

ENTRYPOINT ["/bin/sh", "gsmc.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.simongdium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.smchash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.simongdium"],
		ethashdir:  infos.volumes["/root/.smchash"],
		port:       port,
